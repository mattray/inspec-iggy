...
namespace(:test) do
  ...
  Rake::TestTask.new(:inspec) do |task|
    ...
    sh("wget -O #{tmp_dir}/inspec-aws.tar.gz -nc --tries=10 https://github.com/inspec/inspec-aws/archive/v1.8.1.tar.gz")
    sh("tar -C #{tmp_dir} -xzf #{tmp_dir}/inspec-aws.tar.gz")
    sh("bundle exec inspec exec test/inspec --reporter=progress --input tmp_dir='#{tmp_dir}' resource_dir='#{tmp_dir}/inspec-aws-1.8.1'")
