  ...
  class Negative
    ...
    def self.parse_controls(resources, absolutename, platform)
      ...
      unmatched_resources.each do |unmatched|
        ...
        ctrl.descriptions[:default] = "NEGATIVE-COVERAGE:#{iterator} from the source file #{absolutename}\nGenerated by InSpec-Iggy v#{InspecPlugins::Iggy::VERSION}"
        ctrl.impact = '1.0'

        describe = Inspec::Describe.new
        qualifier = [iterator, {}]
        unresources['qualifiers'].each do |parameter|
          Inspec::Log.debug "Iggy::Terraform::Negative.parse_controls #{iterator} qualifier found = #{parameter} MATCHED"
        ...
        describe.add_test(nil, 'exist', nil, {:negated => true} ) # last field is negated inspec/lib/inspec/objects/describe.rb
